...
class TestTicketBooking:
    ...
    def test_create_new_bookings(self):
        ...
        assert response_body["data"]["eventId"] == self.bookings_data["eventId"], "Incorrect eventId received from the response"
        assert response_body["data"]["customerName"] == self.bookings_data["customerName"], "Incorrect customerName received from the response"
        assert response_body["data"]["customerEmail"] == self.bookings_data["customerEmail"], "Incorrect customerEmail received from the response"
        assert response_body["data"]["status"] == "confirmed", "Incorrect status received from the response"
        assert response_body["message"] == "Booking confirmed!", "Booking is not confirmed"
    ...
    def test_get_all_bookings(self):
        ...
        assert type(all_bookings_response.json()["data"]) is list, "Its not returned all bookings"
        self.cancel_booking(response_body["data"]["id"])
    ...
    def test_single_booking(self):
        ...
        assert get_single_booking.json()["data"]["id"] == response_body["data"]["id"], "Booking id is incorrect"
        self.cancel_booking(response_body["data"]["id"])
    ...
    def test_cancel_booking(self):
        ...
        assert booking_cancel_response.json()["message"] == "Booking cancelled", "Booking is not cancelled"
        all_bookings_response = self.bookings_api.get_all_bookings({})
        assert all_bookings_response.status_code ==  200, f"Expected 200 but received {all_bookings_response.status_code}"
        for booking in all_bookings_response.json()["data"]:
            assert booking["id"] != response_body["data"]["id"], "Booking is not cancelled properly"
