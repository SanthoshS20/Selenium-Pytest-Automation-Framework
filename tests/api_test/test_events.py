...
class TestEvemts:
    ...
    def test_create_new_events(self):
        ...
        assert response_body["data"]["title"] == self.event_data["title"], "Incorrect title received from the response"
        assert response_body["data"]["price"] == str(self.event_data["price"]), "Incorrect price received from the response"
        assert response_body["data"]["totalSeats"] == self.event_data["totalSeats"], "Incorrect totalSeats received from the response"
        assert response_body["message"] == "Event created successfully", "event is not confirmed"
    ...
    def test_get_all_events(self):
        ...
        assert type(all_events_response.json()["data"]) is list, "Its not returned all events"
        self.delete_event(response_body["data"]["id"])
    ...
    def test_single_event(self):
        ...
        assert get_single_event.json()["data"]["id"] == response_body["data"]["id"], "event id is incorrect"
        self.delete_event(response_body["data"]["id"])
    ...
    def test_delete_event(self):
        ...
        for event in all_events_response.json()["data"]:
            assert event["id"] != response_body["data"]["id"], "event is not cancelled properly"
    ...
    def test_update_event(self):
        ...
        assert update_response.json()["data"]["title"] == "welcome", "Title is not updated"
        self.delete_event(response_body["data"]["id"])
    ...
    def delete_event(self, event_id):
        ...
        assert event_delete_response.json()["message"] == "Event deleted successfully", "event is not deleted"
